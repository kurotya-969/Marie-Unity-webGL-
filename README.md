...
戦略の「壊れ方」を定量的に観測するための実験システム

## 概要

本プロジェクトは、ポーカー戦略が「どのように壊れるか」を定量的に観測、安定性を測定するための実験フレームワークです。

### 中心仮説

> 戦略は局面・相手・歪みに対して汎化できず、安定性は平均性能ではなく「壊れ方の分布」で評価される。

GTO/CFRは「前提が成立している世界」でのみ最適であり、前提を連続的に破壊すると、戦略は非連続に崩壊します。

## 特徴
戦術の勝率ではなく「どこでどんな壊れ方をするか＝勝率が急落するか」のプロファイルを測定し、メタ的/工学的に品質を保証。
...
# fork/prについて
自由。ただし知的財産権は放棄せず、またforkに関してはfork先をこちらの改善に役立てる場合がある。（ただしデータが財産であることを鑑み、ここのライブラリ以外の改善に使う場合には作者に連絡、協議してから用いるものとする。）
